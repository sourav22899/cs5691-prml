...
def bayes_classifier(params=None,x=None,loss_matrix=loss_matrix,prior_prob=prior_prob):
  ...
  for i in classes:  
    for j in classes:
      q[i] += loss_matrix[i,j]*multivariate_normal.pdf(x,mean = mu[j],cov= var[j])*prior_prob[j]
...
cs1 = plt.contour(xx, yy, rv1.pdf(pos),color='r')
cs2 = plt.contour(xx, yy, rv2.pdf(pos),color='g')
cs3 = plt.contour(xx, yy, rv3.pdf(pos),color='b')
